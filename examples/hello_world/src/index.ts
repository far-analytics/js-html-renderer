...
const template: Template = doctype()(
  html()(
    head()($html_title, $html_style_sheet, $html_script, $html_inline_script),
    body()(main({ id: "main-content" })($html_main_content), footer({ id: "footer" })())
  )
);
    ...
    if (req.url == "/") {
      ...
      const greetings = ul({ id: "content" })(
        Object.values(worlds).map((greeting: string, index: number) =>
          li({ id: `greeting-${index.toString()}`, class: "greetings" })(greeting)
        )
      );
      ...
      const html_inline_script = script()(sayHello.toString());
