...
type Nodes = (string | Template | symbol | ((...args: Nodes) => Template) | Nodes)[];
...
export class Template {
  ...
  constructor(name: string, attr?: Record<string, string>) {
    this.children = [];
    const isPreamble = name.match(preamble);
    if (!name.match(tagName) && !isPreamble) {
      throw new Error(
        `The tag or preamble named ${name} matches neither the regular expression ${tagName.toString()} nor the regular expression ${preamble.toString()}.`
      );
    }
    let tag = "<" + name;
  ...
  protected addPart(part: string): void {
    if (this.children) {
      ...
      if (typeof last == "string") {
        this.children[this.children.length - 1] = (this.children[this.children.length - 1] as string) + part;
      } else {
  ...
  public render(symbols?: Record<symbol, string | Template | symbol | typeof Template.prototype.collect>): string {
  ...
  public collect(...args: Nodes): this {
    if (this.children) {
      args = args.flat();
  ...
  protected attrToString(attr?: Record<string, string | boolean>) {
    if (!attr) {
      return "";
    }
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      if (!name.match(attributeName)) {
        throw new Error(
          `The attribute name ${name}, does not match the regular expression ${attributeName.toString()}.`
        );
      }
      ...
      if (typeof value == "string") {
        ...
        if (!value.match(attributeValue)) {
          throw new Error(
            `The attribute value ${value}, does not match the regular expression ${attributeValue.toString()}.`
          );
        }
        // eslint-disable-next-line @typescript-eslint/prefer-for-of
